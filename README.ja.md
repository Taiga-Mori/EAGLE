...
- デフォルトの MediaPipe、または選択可能な RetinaFace による顔検出
...
- `yolo26x-pose.pt`
- RetinaFace backend を選択した場合の RetinaFace 学習済み重み
...
- `YOLO object model`
  - YOLO26 detection model（`yolo26n`, `yolo26s`, `yolo26m`, `yolo26l`, `yolo26x`）を選択します。EAGLE は `~/.EAGLE/` に選択中の object 重みだけを残します。
- `Face detection backend`
  - デフォルトは `mediapipe` です。RetinaFace を使いたい場合は `retinaface` を選びます。
- `Off-screen direction backend`
  - `mobileone` です。将来の頭部方向推定モデル backend 追加に備えた設定です。
- `Visualization mode`
  - `both`, `point`, `heatmap`
- `Heatmap alpha`
  - ヒートマップ出力の重ね合わせ強度です。
