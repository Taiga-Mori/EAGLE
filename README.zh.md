...
- 默认使用 MediaPipe 进行人脸检测，也可以选择 RetinaFace
...
- `yolo26x-pose.pt`
- 选择 RetinaFace backend 时所需的 RetinaFace 预训练权重
...
- `YOLO object model`
  - 选择 YOLO26 detection model（`yolo26n`、`yolo26s`、`yolo26m`、`yolo26l` 或 `yolo26x`）。EAGLE 在 `~/.EAGLE/` 中只保留当前选定的 object 权重。
- `Face detection backend`
  - 默认是 `mediapipe`；如果想使用 RetinaFace，可以选择 `retinaface`。
- `Off-screen direction backend`
  - `mobileone`。这个设置是为了以后扩展头部方向估计模型 backend。
- `Visualization mode`
  - `both`, `point`, `heatmap`
- `Heatmap alpha`
  - 热力图输出的叠加强度。
